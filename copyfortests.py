...
#region Classes
class NeonNina:
    def __init__(self, x, y, actual_lvl):
    ...
    def update(self, actual_lvl):
        ...
        if self.actor.y > HEIGHT:
            if self.actor.x > WIDTH: actual_lvl.phase_end()
            else: session.game_over()
        self.animate()
...
class GameSession:
    def __init__(self):
        self.reset()
        self.neon_nina = NeonNina(WIDTH // 2, HEIGHT//2, actual_lvl)
        self.enemies = []
    ...
    def start_game(self):
        self.reset()
        self.state = 'playing'
        self.neon_nina = NeonNina(100, 400, actual_lvl)
...
class Level:
    ...
    def draw(self): screen.draw.filled_circle(self.next_phase, 20, "yellow")
    def phase_end(self, player_pos):
        dx = player_pos[0] - self.next_phase[0]
        dy = player_pos[1] - self.next_phase[1]
        distance = (dx*dx + dy*dy) ** 0.5
        if distance < 30:
            phase = next_phase
        return distance < 30
actual_lvl = Level(1, 'bg_lvl1')
#endregion
#region Session
session = GameSession()
...
def update():
    if session.state == 'playing':
        session.neon_nina.update(actual_lvl)
        for d in drones:
            d.update()
        if keyboard.h:
