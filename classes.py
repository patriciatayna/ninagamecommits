...
def change_level():
    global current_level_index, platforms, drones, current_level
...
def update():
    if session.state == 'playing':
        ...
        if keyboard.h:
            pass  # session.neon_nina.hacking() placeholder

def draw_hud():
    # Barra de vida da Neo-Nina (topo esquerdo)
    x, y = 20, 20
    width, height = 200, 24
    hp_ratio = session.neon_nina.hp / 100
    screen.draw.rect(Rect((x, y), (width, height)), "white")  # contorno
    screen.draw.filled_rect(Rect((x+2, y+2), (int((width-4)*hp_ratio), height-4)), "red")

    # Texto "HP"
    screen.draw.text("HP", (x - 30, y), fontsize=24, color="white")

    # Ícones dos aliados hackeados (topo esquerdo, próximo da barra de vida)
    icon_x = x + width + 40
    icon_y = y + height // 2
    spacing = 40
    for i, ally in enumerate(session.neon_nina.allies):
        screen.draw.image('ally_icon', (icon_x + i*spacing, icon_y), anchor=("center", "center"))

    # Indicador da fase atual (topo centro)
    screen.draw.text(f"Phase {current_level.phase}", center=(WIDTH // 2, 30),
